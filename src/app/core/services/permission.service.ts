...
export class PermissionService extends HttpService {
   ...
   * @returns 
   */
    async getPermissionById(data:any){
      let events: Array<any> = new Array<any>();
        const id = data;
        const resp = await firstValueFrom(this.get(environment.apiUrl,`/staexped/permisos/${id}`));
        const arrayVacio = (resp) => !Array.isArray(resp) || resp.length === 0;
        if (resp){
            events = resp.data.map((item: any) => {
              return Permission.mapFromObject(item);
            })
        return events;
        }
      }
  ...
  async storePermission(data:any){
    try {
      if(data.id){
        const id = data.id;
        delete data.id;
        await firstValueFrom(this.put(environment.apiUrl,`/staexped/permisos/actualizar/${id}`, data));
        this.toastrService.success('Datos actualizado con exito.');
        
      }else{
        const resp = await firstValueFrom(this.post(environment.apiUrl,'/staexped/permisos', data));
   ...
   * @returns 
   */
   async storePermissionDelete(data:any){
    try {
        const id = data;
        //delete data.id;
        await firstValueFrom(this.delete(environment.apiUrl,`/staexped/permisos/eliminar/${id}`, data));
