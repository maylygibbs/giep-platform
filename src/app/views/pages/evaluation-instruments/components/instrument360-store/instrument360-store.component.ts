...
@Component({
  selector: 'app-instrument360-store',
  templateUrl: './instrument360-store.component.html',
  styleUrls: ['./instrument360-store.component.scss']
})
export class Instrument360StoreComponent extends BaseComponent implements OnInit {
  ...
  async deleteSection(section: Section) {
    ...
    if(section.id){
      result = await this.evaluationInstrumentsService.deleteSection(section.id)
    }
    if(result){
 ...
 */
  ...
  async loadUsersByRoles() {
    ...
    this.users = await this.evaluationInstrumentsService.getUsersByRoles(this.arrayToString(this.selectedRoles, '|'));
